...
2. Экспортируете кэш через кнопку «Экспорт кэша» в UI (или `GET /api/export-cache`) — скачается `.tar.gz` архив.
3. Переносите архив на офлайн-машину и импортируете через кнопку «Импорт кэша» (или `POST /api/import-cache`).
4. На офлайн-машине запускаете сервер и указываете `GOPROXY` на него.
...
Скачивает архив `tar.gz` с содержимым кэша:
...
```bash
curl -X POST http://127.0.0.1:8080/api/import-cache -F archive=@go-offline-cache.tar.gz
```
