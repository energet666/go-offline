...
- Prefetch по содержимому `go.mod`: текст можно вставить, выбрать файл или перетащить его в форму.
- Из UI prefetch всегда **рекурсивный**: тянется весь граф зависимостей (build list) плюс `go.mod` всех рёбер графа, чтобы сборка на офлайн-машине не упёрлась в отсутствующий файл. Через API глубину можно ограничить (`recursive`).
...
- **Самообновление**: карточка «Обновление приложения» в UI проверяет сборку в GitHub Releases и заменяет бинарь на месте с перезапуском.
...
`version` можно не указывать — тогда возьмётся `latest`, и закрепится уже конкретная версия, к которой запрос разрезолвился. Если передать путь пакета, а не модуля, сервер сам найдёт модуль, которому этот пакет принадлежит.

`recursive` управляет глубиной: `true` — весь граф зависимостей, `false` (значение по умолчанию, если поле опущено) — только сам модуль. UI всегда отправляет `true`; для переносимого офлайн-кэша это единственный полезный режим.

Загрузка одна на весь сервер: пока идёт одна, вторая вернёт `409` с `{"error":"download already in progress"}`.
...
Содержимое `go.mod` передаётся целиком, как текст. `replace` на локальные пути (`./...`, абсолютные) отбрасываются: этих директорий здесь нет, а без них `go` отказывается читать граф — остальные зависимости при этом качаются нормально. `recursive:false` ограничит загрузку прямыми `require`.

В UI это блок **«Prefetch из go.mod»**: содержимое можно вставить в поле, выбрать файл ссылкой **«выбрать файл»** или перетащить `go.mod` прямо на поле ввода.
...
```bash
# Что опубликовано в релизе и стоит ли обновляться
curl http://127.0.0.1:8080/api/self-update/check

# Версия текущего процесса (в сеть не ходит)
curl http://127.0.0.1:8080/api/version

# Скачать новую сборку, заменить бинарь и перезапуститься
curl -X POST http://127.0.0.1:8080/api/self-update/apply
```

Проверка ходит в интернет, поэтому выполняется только по явному запросу — на офлайн-машине приложение само в сеть не лезет. Обновление занимает единственный слот фоновых задач: пока идёт prefetch, `apply` вернёт `409`.

Как это устроено: CI публикует бинари и манифест `latest.json` в скользящий релиз `nightly`. Версия — это commit sha, вшитый в бинарь через `-ldflags`; тег `nightly` не меняется и для сравнения не годится. Скачанный файл проверяется по SHA256 из манифеста, затем подменяет текущий исполняемый файл, и процесс перезапускается с теми же аргументами. На Windows работающий `.exe` перезаписать нельзя, поэтому старый уезжает в `*.exe.old` и удаляется при следующем старте.
...
- `-update-url` база, откуда берутся `latest.json` и бинари для самообновления (по умолчанию релиз `nightly` на GitHub)
...
- **`go.mod` требует более новый Go.** Тулчейны намеренно не скачиваются (`GOTOOLCHAIN=local`): это ~60 МБ под платформу текущей машины, они попали бы в экспортируемый кэш и оказались бы бесполезны на офлайн-машине с другой ОС/архитектурой. Обновите сам бинарник `go` (или укажите другой через `-go-bin`).
- **Часть модулей не скачалась.** Загрузка графа не падает целиком из-за одного недоступного модуля: он пишется в лог как `warn: skip ...`, а в конце появляется строка `graph download completed: attempted=N failed=M`.
- **Логи обрезаются.** В памяти хранится не более 1000 строк: сохраняются первые 200 и последние, вместо выброшенных вставляется строка `… пропущено строк: N …`.

## Разработка

```bash
make build   # сборка UI + бэкенда (UI обязательно первым — он вшивается в бинарник)
make run     # сборка и запуск
make test    # go test ./...
make clean   # удалить бинарник и собранный UI (node_modules остаётся)
```

Фронтенд собирается Vite в `internal/4_presentation/http/web` и встраивается в бинарник через `go:embed`, поэтому после изменений в `frontend/` нужен полный `make build`. Подробности про dev-режим — в [frontend/README.md](frontend/README.md), про архитектуру — в [AGENTS.md](AGENTS.md) и [ddd_architecture.md](ddd_architecture.md).
